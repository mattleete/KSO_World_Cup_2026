...
- Instead, a GitHub Action (`.github/workflows/refresh-fixtures.yml`, repo root, runs on the **`main`** default branch) caches `/matches` into the `match_feed` Supabase table (one row, `id='wc2026'`) using the **service_role** key, every ~5 min. NOTE: GitHub's `schedule` event is unreliable for short intervals (it fires every few HOURS, not every 5 min), so each run is a **long-lived job that loops internally every 5 min for ~5h40m**; `concurrency` (cancel-in-progress: false) makes a freshly-scheduled run queue and take over when the current job ends, keeping refreshes ~continuous. ~286 API calls/day (under the 500/day plan). Repo is public, so Actions minutes are unlimited. If it ever stalls, **Actions → Run workflow** restarts the chain.
