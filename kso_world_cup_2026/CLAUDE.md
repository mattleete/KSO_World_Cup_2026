...
- Instead, a scheduled GitHub Action (`.github/workflows/refresh-fixtures.yml`, repo root, runs on the **`main`** default branch) fetches `/matches` every 5 min (288/day, under the 500/day plan; 5 min is GitHub's minimum cron interval) and upserts the raw array into the `match_feed` Supabase table (one row, `id='wc2026'`) using the **service_role** key. (Repo is public, so Actions minutes are unlimited.)
